...
void setup(){
    ...
    setDefaultNodeNameShow(true, SOUTHWEST);
...
int roots[ROOTS];
int kids[KIDS*ROOTS];
int gKids[GKIDS*GGKIDS*ROOTS];
int ggkids[GGKIDS*GKIDS*KIDS*ROOTS];
...
void buildTree(){
     for (int i = 0; i < ROOTS; i++){
         ...
         for (int j = 0; j < KIDS; j++){
             kids[i*KIDS+j] = i;
             makeNode(kids[KIDS*i+j]);
             addChild(roots[i], kids[KIDS*i+j]);
             for (int k = 0; k < GKIDS; k++){
                 gKids[(KIDS*i+j)*GKIDS+k] = i*KIDS + j;
                 makeNode(gKids[(KIDS*i+j)*GKIDS +k]);
                 addChild(kids[KIDS*i+j], gKids[(KIDS*i+j)*GKIDS +k]);
		 for (int l = 0; l < GGKIDS; l++){
                     ggkids[((KIDS*i +j)*GKIDS + k)*GGKIDS +l] = (KIDS*i +j)*GKIDS + k;
                     makeNode(ggkids[((KIDS*i+j)*GKIDS +k)*GGKIDS+l]);
                     addChild(gKids[(KIDS*i+j)*GKIDS + k], ggkids[((KIDS*i+j)*GKIDS +k)*GGKIDS+l]);
                 }
             }
         }
     }
     ...
     makeEdge(kids[3], gKids[1]);
     makeEdge(gKids[4], gKids[5]);
     makeEdge(roots[1], gKids[9]);
...
*/
    ...
    for (int i = 0; i < GGKIDS*GKIDS*KIDS*ROOTS; i++)
    	ScriptManager::relay("HigraphManager", "setNodeValueColor", ggkids[i], RED);
