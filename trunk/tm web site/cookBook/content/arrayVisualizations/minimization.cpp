...
void setup(){
    ...
    ScriptManager::relay("HigraphManager", "setDefaultShowNodeName", TRUE,CENTER);
...
// the adjacency matrix equal to true
        ...
        graph[nodeId + 1][nodeId] = true; /*#TS*/ makeEdge(p[v], n[v], BLACK);/*#/TS*/
}
...
// corresponding six elements in the adjacency matrix equal to true
        ...
        graph[nodeId][nodeId + 1] = true;
        graph[nodeId + 1][nodeId] = true; /*#TS*/ makeEdge(f[c], s[c], BLUE);/*#/TS*/
        graph[nodeId][nodeId + 2] = true;
        graph[nodeId + 2][nodeId] = true; /*#TS*/ makeEdge(f[c], t[c], BLUE);/*#/TS*/
        graph[nodeId + 1][nodeId + 2] = true;
        graph[nodeId + 2][nodeId + 1] = true; /*#TS*/ makeEdge(s[c], t[c], BLUE);/*#/TS*/
...
// in the adjacency matrix equal to true
        ...
        for (int l = 0; l < 3; l++) {
            ...
            graph[v][nodeId + l] = true; /*#TS*/ makeEdge(mapNode(nodeId + l), mapNode(v), GREEN);/*#/TS*/
        }
}
...
void main( ) { /*#TS*/  setup(); /*#/TS*/
    ...
    for (int c = 0; c < ND; c++) {
    	connectClauseGadget(c); /*#TS*/ colorAllNodes(BLACK, WHITE, BLACK); colorAllEdges(BLACK);/*#/TS*/
    } /*#TS*/ makeString("blackNodes", "black nodes signify ...", 800, 50, BLACK);/*#/TS*/
