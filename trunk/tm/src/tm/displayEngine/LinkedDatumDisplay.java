...
//    private boolean linked = false; //true if already linked into display
    ...
    private Attachment[] attachments = new Attachment[LT+1];
    private Link myLink=null;       // Only not null if represents a pointer
 ...
 */
    ...
    public LinkedDatumDisplay(Datum datum, DataDisplayView top, int level){
        ...
        for (int i = 0; i <=LT; i++)
            attachments[i] = new Attachment(this, i);
        if (datum instanceof PointerInterface) {
            ...
            if (deref != null) {
                ...
                if (kid != deref) { // Pointing inside of new LDD
                    ...
                    myLink = new Link(attachments[RCI],
            ...
            else myLink = new Link(attachments[RCI], null);
        }
 ...
 */   
    ...
    public Point getInterimLocation() { 
        return new Point( morph(oldLocation.x, extent.x),
                          morph(oldLocation.y, extent.y) );
    }
 ...
 // Get co-ordinates of attachment point
    Point getAPoint(int i){
        ...
        switch(i){
            ...
            case RU:    // Right upper   
                point.x += extent.width;
                point.y += extent.height/4;
                break;
            case RL:    // Right lower
                point.x += extent.width;
                point.y += (3*extent.height)/4;
                break;
            case LC:    // Left center
                point.x += nameBox.getStringLeft() - GAP_LEFT;
                point.y += extent.height/2;
                break;
            case LU:    // left upper
                point.x += nameBox.getStringLeft() - GAP_LEFT;
    ...
    Point getAStub(int i){
        Point point = getAPoint(i);
        if (i == RCI)
            point.x += POINTER_STUB + INSET_RIGHT;
        else if (i < LC)  // Right side
            point.x += POINTER_STUB;
        else
            point.x -= POINTER_STUB;
        return point;
    }
 ...
 */   
 ...
 //       int x = morph(oldLocation.x,extent.x);         // left edge
 //       int y = morph(oldLocation.y,extent.y);         // top corner
...
// No address box in linked view	
		...
		Color color = (myDatum.getHighlight() == Datum.HIGHLIGHTED) ?
		    Color.yellow :  (grayOut ? Color.lightGray : null);
    ...
    public void drawLinks(Graphics screen){
        D3Iterator iterator = getIterator();    // get one of my iterators
        while (!iterator.atEnd()){
            Link link = ((LinkedDatumDisplay)iterator.getNode()).getLink();
            if (link != null)
                link.draw(screen);
            iterator.increment();
        }
