...
import regex.model.ISelectable;
 ...
 * @author Xiaoyu Guo
 */
public abstract class RegexNodeView 
extends NodeView<RegexPayload, RegexEdgeLabel, RegexHigraph, RegexWholeGraph, RegexSubgraph, RegexNode, RegexEdge>
implements ISelectable, IHoverable, IRotatable, IInteractable {
    /** Handles the LayoutManager */
    private final BTVar<RegexNodeLayoutManager> layoutManagerVar;
    ...
    protected void drawSelection(Graphics2D screen) {
        if (this.isSelected()){
            ...
            Iterator<ZoneView<RegexPayload, RegexEdgeLabel, RegexHigraph, RegexWholeGraph, RegexSubgraph, RegexNode, RegexEdge>> iter = this.getZoneIterator();
            while(iter.hasNext()){
                ZoneView<RegexPayload, RegexEdgeLabel, RegexHigraph, RegexWholeGraph, RegexSubgraph, RegexNode, RegexEdge> zone = iter.next();
                screen.drawRect(
                    (int)zone.getNextX(),
                    (int)zone.getNextY(),
    ...
    public void doLayout(double px, double py){
        getLayoutManager().layoutNode(this, px, py);
    }
    
    public void doTransition(){
        for(RegexDropZone zone : this.zoneMap.values()){
            zone.doTransition();
        }
        super.doTransition();
    }
    ...
    /**
     * Gets the corresponding {@link RegexNodeLayoutManager} of this view
     * @return the specified LayoutManager, default if not specified.
     */
    public RegexNodeLayoutManager getLayoutManager(){        
    ...
    public void setLayoutManager(RegexNodeLayoutManager manager){
        this.layoutManagerVar.set( manager ) ;
    }
    
    private RegexViewFactory factory;
    
    /**
     * @param regexViewFactory
     */
    public void setNodeViewFactory(RegexViewFactory value) {
        this.factory = value;
    }
    
    public RegexViewFactory getNodeViewFactory(){
        return this.factory;
    }
    /**
     ...
     * @return
     */
    protected abstract RegexNodeLayoutManager getLayoutHelper();
    
    /**
     * Gets the ith child and cast it to {@link regex.view.RegexNodeView}
     * @param i
     * @return
     */
    public RegexNodeView getRegexChild(int i){
        NodeView<RegexPayload, RegexEdgeLabel, RegexHigraph, RegexWholeGraph, RegexSubgraph, RegexNode, RegexEdge> kid = this.getChild(i);
        if(kid instanceof RegexNodeView){
            return (RegexNodeView)kid;
        } else {
            return null;
        }
    }
    ...
    public final void setHoverOff(){
        ...
        for(RegexDropZone dz : this.zoneMap.values()){
            if(dz.isHoverOn()){
                dz.setHoverOff();
            }
        }
    ...
    /** Zone Handling */
    protected Map<String, RegexDropZone> zoneMap;
    
    public void addZone(RegexDropZone zone){
        super.addZone(zone);
        this.zoneMap.put(zone.getId(), zone);
    }
    
    public void removeZone(RegexDropZone zone){
        super.removeZone(zone);
        this.zoneMap.remove(zone.getId());
    }
    
    public final void removeZones(){
        Iterator<ZoneView<RegexPayload, RegexEdgeLabel, RegexHigraph, RegexWholeGraph, RegexSubgraph, RegexNode, RegexEdge>> itor = this.getZoneIterator();
        while(itor.hasNext()){
    ...
    public RegexDropZone findZone(String id){
    ...
    public void moveZone(
        ZoneView<RegexPayload, RegexEdgeLabel, RegexHigraph, RegexWholeGraph, RegexSubgraph, RegexNode, RegexEdge> zone)
    {
        Assert.check(zone instanceof RegexDropZone);
        this.getLayoutManager().layoutZones(this, (RegexDropZone)zone);
    }
     ...
     */
    protected final RegexDropZone makeAndAddDropZone(String id, int number){
        if(this.mayAddZones() && this.findZone(id) == null){
            RegexDropZone zone = this.getNodeViewFactory().makeDropZone(this);
            zone.setId(id);
    ...
    protected final RegexDropZone makeAndAddInsertChildDropZone(String id, int number){
        if(this.mayAddZones() && this.findZone(id) == null){
            RegexDropZone zone = this.getNodeViewFactory().makeInsertChildDropZone(this);
            zone.setId(id);
            zone.setNodeNumber(number);
            this.addZone(zone);
            return zone;
    ...
    /* (non-Javadoc)
     * @see regex.view.IRotatable#getCurrentDirection()
     */
    @Override
    ...
    /* (non-Javadoc)
     * @see regex.view.IRotatable#setDirection(regex.model.Direction)
     */
    @Override
    public void setDirection(Direction newDirection){
        if(! newDirection.equals(this.headingDirection)){
            ...
            for(int i = 0; i < this.getNumChildren(); i++){
                this.getRegexChild(i).setDirection(newDirection);
            }
        }
    }
    
    /* (non-Javadoc)
     * @see regex.view.IRotatable#reverseDirection()
     */
    @Override
    public final void reverseDirection(){
        this.headingDirection = this.headingDirection.getReverseDirection();
        for(int i = 0; i < this.getNumChildren(); i++){
            this.getRegexChild(i).reverseDirection();
        }
    }
}
