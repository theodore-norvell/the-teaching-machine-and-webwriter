...
Overview:
This class represents Java variables (or lvalues)

Review:          xxxx xx xx     xxxxxxxxxxxxx
*******************************************************************************/

public class TyRef extends TyAbstractRef implements TyJava {
    public TyRef() { super(); }
    public TyRef(TypeNode pointeeTp) { super(pointeeTp); }

    public AbstractDatum makeMemberDatum(VMState vms, int address,
                                         AbstractDatum parent, String name) {
        RefDatum d = new RefDatum(address, parent, vms.getMemory(), name, this,
                                  vms.getStore(), vms.getTimeManager());
        return d;
    }

    public AbstractDatum makeDatum(VMState vms, MemRegion mr, String name) {
        int address = mr.findSpace(RefDatum.size);
        RefDatum d = new RefDatum(address, null, vms.getMemory(), name, this,
                                  vms.getStore(), vms.getTimeManager());
    ...
    public void addToEnd(TypeNode t) {
        Assert.check( t instanceof TyPrimitive || t instanceof TyPointer );
        super.addToEnd(t);
    }


    public String typeId() { return ((TyJava)getPointeeType()).typeId(); }
    public String elementId(){ return ""; }
    ...
    public String toString(){return "reference to" + typeId();}
}