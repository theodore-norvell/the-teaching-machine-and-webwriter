...
/*  constants for categorizing where the mouse clicked. See checkClick() method
*/
...
/*  static value providing functions. Most of them provide
    ...
    to sub-class. They should be over-ridden in the concrete classes. 
*/
    public static int getNameX(int nestLevel){ return 0; }
    public static int getValueX(int nestLevel){ return 0; }
    public static int getAddressX(int nestLevel){ return 0; }
    public static int getDescenderX(int nestLevel){ return 0; }
    public static int getExpanderX(int nestLevel){ return 0; }
    public static int getCompoundXLeft(int nestLevel){ return 0; }
    public static int getCompoundXRight(int nestLevel){ return 0; }
...
*/
    protected DataDisplayView myDataView = null;   // Change this to a memoryDisplay
...
/*  If the associated datum is the child of another datum, specifies whether it is 
    a oldest, middle, or youngest child. Leaders are drawn slightly differently
 */
    protected int birthOrder;
    
 /* The extent of the displayDatum in its parent's drawing (pixel) co-ordinates
 */
...
/* 3/08/2001: Major change. Since every Datum in a region being displayed has
    to have a DatumDisplay attached, whether or not it is actually being
    ...
    them considerable easier since currently any Datum that is discovered to have no
    DatumDisplay attached gets one attached. The point of discovery may not have easy
    access to the nesting level.
    Further, when a DatumDisplay is constructed, it initializes expander boxes per
...
//        System.out.println("0: datum is " + datum + " and displayView is " + displayView);
...
// Service routine to return the DatumDisplay associated with a Datum  
    ...
    public Point getLocation(){return new Point(extent.x, extent.y);}
    ...
    public void move(int x, int y){
        ...
        /* move kids, because all datums are located absolutely but NOT the expander
            which is located relatively
        */
 ...
 /* Resize is responsible for locating all subcomponents properly. In this default
    code, components are laid out from left to right starting with the nameBox. After
    it comes a gap for expanders (whether or not the DatumDisplay object has one) 
    followed by the valueBox, finally by an address box if one exists. Resize
...
/*  A mouse click has occurred, probably inside my box. I need to check if my
    ...
    of their expanders has been hit.
    Expander boxes will be in different locations so only 1 can be hit.
    Any hit requires a complete refresh of the base display.
*/
    public void mouseClicked(Point p){
        if(extent.contains(p)){
            ...
            if (myExpander == null)
            	...
            	if (myExpander.contains(p, getLocation())){ // clicked on expander
            ...
            if (clickSelect) {
            	selected = !selected;
            	myDataView.refresh();  // Changed June 8, 2007 from a system wide refresh
            }
        }
    }

/* Recursive check on whether expander box is hit. If so changes state of box and
    requests any height changes necessary. DON'T BELIEVE THIS IS USED
    */
    ...
    boolean isGrayedOut(){return grayOut;}
        ...
        if (myExpander.getExpanded()) { // Draw leaders, descenders and kids
            ...
            for (int i = 0; i < kids; i++) {
                ...
                if (i > 0) { // Not the first kid
                    ...
                    yEnd = localR.y + (i == (kids-1) ? baseHeight/2 : depth); // last descender stops at leader
