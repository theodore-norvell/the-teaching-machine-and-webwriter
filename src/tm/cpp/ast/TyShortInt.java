...
import tm.cpp.datum.ShortDatum;
import tm.interfaces.TypeInterface;
import tm.utilities.Assert;
import tm.virtualMachine.MemRegion;
import tm.virtualMachine.VMState;

public class TyShortInt extends TypeNode implements TyIntegral, TyCpp {

	private TyShortInt() {
		super() ; }

	private static TyShortInt [] qualified = new TyShortInt [4];

	/**
	 * Returns a <code>TyShortInt</code> instance with default (none/auto) 
	 * cv-qualification.
	 * @return a <code>TyShortInt</code> instance
	 * @see Cpp.Analysis.SpecifierSet
	 */
	public static TyShortInt get () {
		return get (0);
	}

	/**
	 * Returns a <code>TyShortInt</code> instance with the indicated 
	 * cv-qualification.
	 ...
	 * @see Cpp.Analysis.SpecifierSet
	 */
	public static TyShortInt get (int cv_qual) {
		TyShortInt instance = null;
		if (cv_qual >= qualified.length) {
			Assert.apology (INVALID_CV_QUALIFICATION);
		} else { 
			if (qualified [cv_qual] == null) {
				qualified [cv_qual] = new TyShortInt ();
	...
	public boolean equal_types( TypeInterface t ) {
		return t instanceof TyShortInt ; }
	
	public String getTypeString () { return "short int"; }
