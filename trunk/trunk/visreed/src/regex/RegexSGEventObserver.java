...
import regex.swing.RegexNodeTransferObject;
import regex.view.*;

public class RegexSGEventObserver
extends SubgraphEventObserver<RegexPayload, RegexEdgeLabel, RegexHigraph, RegexWholeGraph, RegexSubgraph, RegexNode, RegexEdge> {
     ...
     * @param wg
     */
    public RegexSGEventObserver(IGraphContainer frame, RegexWholeGraph wg){
    ...
    private RegexHigraphView mySubgraphView ;
    private RegexWholeGraph myWholeGraph;
    private IGraphContainer graphContainer;
    private IHoverable lastHoveringOn;
    
    private StringBuilder sb = new StringBuilder();
    
    private RegexNodeView getTopNodeView(Stack<ComponentView<RegexPayload, RegexEdgeLabel, RegexHigraph, RegexWholeGraph, RegexSubgraph, RegexNode, RegexEdge>> stack){
        if(stack == null || stack.size() == 0){
            return null;
        }
        if(stack.peek() instanceof RegexNodeView){
            return (RegexNodeView)stack.peek();
        } else if (stack.peek() instanceof RegexDropZone){
            return (RegexNodeView)(((RegexDropZone)stack.peek()).getAssociatedComponent());
        } else {
            return null;
        }
    }
    
    private IHoverable getTopHoverable(Stack<ComponentView<RegexPayload, RegexEdgeLabel, RegexHigraph, RegexWholeGraph, RegexSubgraph, RegexNode, RegexEdge>> stack){
        if(stack == null || stack.size() == 0){
            return null;
        }
    ...
    private String getTopNodeDescFromStack(Stack<ComponentView<RegexPayload, RegexEdgeLabel, RegexHigraph, RegexWholeGraph, RegexSubgraph, RegexNode, RegexEdge>> stack){
        sb.setLength(0);
        if(stack == null || stack.size() == 0){
            return "";
        }
        ...
        sb.append("<");
        ComponentView<RegexPayload, RegexEdgeLabel, RegexHigraph, RegexWholeGraph, RegexSubgraph, RegexNode, RegexEdge> v = stack.firstElement();
        if(v instanceof RegexNodeView){
            RegexNodeView rnv = (RegexNodeView)v;
            RegexPayload pl = rnv.getNode().getPayload();
            sb.append(pl.getDescription());
        } else if (v instanceof RegexDropZone){
            RegexDropZone rdz = (RegexDropZone)v;
            sb.append("dropzone(");
            sb.append(rdz.getId());
            sb.append(") for ");
            
            RegexNodeView rnv = (RegexNodeView)(rdz.getAssociatedComponent());
            RegexPayload pl = rnv.getNode().getPayload();
            sb.append(pl.getDescription());
        }
        sb.append(">");
    ...
    private boolean isSelectedSource(){
        ...
        List<RegexNodeView> selection = this.getSelectedViews();
        for(RegexNodeView nv : selection){
    ...
    @Override
    public void movedOver(Stack<ComponentView<RegexPayload, RegexEdgeLabel, RegexHigraph, RegexWholeGraph, RegexSubgraph, RegexNode, RegexEdge>> stack, MouseEvent e) {
//        System.out.println( getTopNodeDescFromStack(stack) + " : Moved over") ;
    ...
    @Override
    public void dragged(Stack<ComponentView<RegexPayload, RegexEdgeLabel, RegexHigraph, RegexWholeGraph, RegexSubgraph, RegexNode, RegexEdge>> stack, MouseEvent e) {
    ...
    @Override
    public void clickedOn(Stack<ComponentView<RegexPayload, RegexEdgeLabel, RegexHigraph, RegexWholeGraph, RegexSubgraph, RegexNode, RegexEdge>> stack, MouseEvent e) {
//        System.out.println( getTopNodeDescFromStack(stack) + " : Clicked on " + e.getClickCount() + " times") ;
        
        if(e.getClickCount() == 1){
            // handle click selection
            RegexNodeView view = getTopNodeView(stack);
            if(view == null){
                // clear selection
    ...
    @Override
    public void pressedOn(Stack<ComponentView<RegexPayload, RegexEdgeLabel, RegexHigraph, RegexWholeGraph, RegexSubgraph, RegexNode, RegexEdge>> stack, MouseEvent e) {
        this.dragSourcePoint = e.getPoint();
        this.dragTargetPoint = null;
        
        // handle marquee selection
        if(e.getButton() == MouseEvent.BUTTON1){
            this.lastButton = MouseEvent.BUTTON1;
            this.marqueeStartingPoint = e.getPoint();
//            System.out.println("Pressed: starting = (" + this.marqueeStartingPoint + ")");
     ...
     */
    @Override
    public void releasedOn(Stack<ComponentView<RegexPayload, RegexEdgeLabel, RegexHigraph, RegexWholeGraph, RegexSubgraph, RegexNode, RegexEdge>> stack, MouseEvent e) {
//        System.out.println( "Release on, position = (" + e.getPoint() + ")" ) ;
        ...
        if(this.lastButton == MouseEvent.BUTTON1){
            ...
            this.mySubgraphView.clearMarquee();
            this.graphContainer.repaint();
        } else if (this.lastButton == MouseEvent.BUTTON2){
            if(this.isSelectedSource()){
     ...
     * @return
     */
    private RegexNodeView getSelectedView(){
        RegexNodeView result = null;
        if( this.myWholeGraph.getSelectionNodes().size() == 0 ){
            // do nothing
        } else {
            RegexNode node = this.myWholeGraph.getSelectionNodes().get(0);
            result = (RegexNodeView) this.mySubgraphView.getNodeView(node);
        }
        return result;
    }
     ...
     * @return
     */
    private List<RegexNodeView> getSelectedViews(){
        List<RegexNodeView> result = new ArrayList<RegexNodeView>();
        for(RegexNode node : this.myWholeGraph.getSelectionNodes()){
            result.add((RegexNodeView)this.mySubgraphView.getNodeView(node));
        }
        return result;
    }
    
    private int getNumSelection(){
...
//            result = false;
            ...
            result |= supportObj.isDataFlavorSupported( RegexNodeTransferObject.theNodeDataFlavor ) ;
...
//                result = false ;
    ...
    public boolean importData(
        ...
        List<RegexNode> data = new ArrayList<RegexNode>();
        try {
            data.add( (RegexNode)support.getTransferable().getTransferData(RegexNodeTransferObject.theNodeDataFlavor) );
        } catch (Exception e) {
