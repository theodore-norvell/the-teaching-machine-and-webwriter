...
// Accessors
	MyString name() const;	// return reference
	int allocation() const;	// Disk space allowed
	MyString password() const;	// return value
...
/*#TC*/	int main(){
User mpbl("Michael Bruce-Lockhart", "mpbl", "bftplx");
	User theo("Theo Norvell", "theo", "charl1eB");
    //User mpblClone(mpbl);
	mpbl.setAllocation(100);
	mpblClone.setPassword(MyString("unlucky"), MyString("unlucky"));
