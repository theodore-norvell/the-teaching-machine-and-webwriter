...
//		    DatumDisplay dd = (LinkedDatumDisplay)DatumDisplay.getAssociated(kid,myDataView);
        LinkedD3Iterator iterator  = myGenerator.getIterator(this);   // Each compound node has its own iterator
...
/*	    Point scroll = new Point(myWorkPane.getVerticalScrollBar().getValue(),
	    					myWorkPane.getHorizontalScrollBar().getValue() );*/
	    
	    int step;
	    if(animate)
	        step = LinkedDatumDisplay.LAST_STEP;           // punting - crude two-step animation
        ...
        while (!iterator.atEnd()) {   
            ...
            dd.setStep(step);
     /*       if (dd.getInterimLocation().y >= scroll.y - h ) */ // allow for partial display
            dd.draw(screen);
     /*       if ((dd.getInterimLocation().y + h) > (scroll.y + myWorkPane.getSize().height)+2)
                break; */
            iterator.increment();
		}
