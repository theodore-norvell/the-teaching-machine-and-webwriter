...
function insertCode(relativeURL, buttonSet, className, configurationFile, wwSelection, tmSelection){
	...
	if (wwSelection != null) setSelector(wwSelection);
	else setSelector("");
//	alert("insertCode.");
...
//    alert("InsertCode: JavaURL is " + getJavaURL(theURL));
	rawCode = loadCode(theURL, configurationFile, tmSelection);
...
/* Hoisted out of invokeCode in June, 2007, to allow code sharing with new runCode routine.
	This funcion fetches the raw code for the new example. It also loads the TM version of
...
*/
function loadCode(theURL, configurationFile, tmSelection){
	var javaURL = getJavaURL(theURL);
	rawCode = fileToString(javaURL);
	currentCode++;		// track no of examples on this page
	exampleURL[currentCode] = javaURL;
...
var displayExpression = "";	// expression to control display of selections 
var tagSet = {};

function setSelector(expression){
	displayExpression = expression;
}

function addTag(tag){
	tagSet.tag = true;  // actually, value doesn't matter, only existence of the property
}

function removeTag(tag){
	if (tag in tagSet) delete tagSet.tag;
}

function hideCode(){
	var display = true;  // default
...
/* 10.15.2002: Added switchLetter to allow single piece of code to be displayed
	...
	04.02.2009: Changed to A (for add tag). On TM side, effect of DL and HL
	    (where L can be any letter) was to add the letter L as a tag to the
		selection. Displaying and hiding of selections was to some extent
		managed separately. Now we will separate taging and displaying completely.*/
		...
		case "D":	// display (opposite of hide) - deprecated
		case "T":
			var letter = annotation.substr(1,1);
			if(closing) removeTag(letter)
			else addTag(letter);
/* 10.15.2002: Added switchLetter to allow single piece of code to be displayed
	multiple times with different pieces showing */
			if (hideCodeSwitch !="all"){
