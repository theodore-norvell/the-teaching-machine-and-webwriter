...
/*#DB*/ // A function to set every element of an array to value
...
/* A "good" copy constructor, that is one which does
  a proper deep copy. Note contrast to constructor
  */
Array::Array(const Array& original){
	 // request same amt. of space
	if (mpData = new double[original.mSize]){
		mSize = original.mSize;	// success! have space
		for (int i = 0; i < mSize; i++)	// Copy heap data
			*(mpData+i) = *(original.mpData+i);
	}
	else mSize = 0;		// Oops! Note no space.  
}
