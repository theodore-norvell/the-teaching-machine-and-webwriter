...
function writeHeader(theDocument, path){
	if (!theDocument || theDocument == "") {
		theDocument = document;
		path = "";
	}
	theDocument.write('<meta NAME="Generator" CONTENT="DreamWeaver MX2004">');
	theDocument.write('<meta NAME="Author" CONTENT="Michael Bruce-Lockhart">');
	theDocument.write('<meta NAME="copyright" CONTENT="&copy; 2004, Michael Bruce-Lockhart">');
	theDocument.write('<meta NAME="resource-type" CONTENT="document">');
	theDocument.write('<meta NAME="distribution" CONTENT="global">');
	theDocument.write('<link rel="stylesheet" type="text/css" media="screen" title="lecture" href="');
		theDocument.write(path, 'site_resources/stylesheets/clecture.css" title="C tutoring Projector stylesheet">');
	theDocument.write('<link rel="alternate stylesheet" type="text/css" media="screen" title="notes" href="');
		document.write(path, 'site_resources/stylesheets/cstudy.css" title="C tutoring Screen stylesheet">');
	theDocument.write('<link rel="stylesheet" type="text/css" media="print" href="');
		document.write(path, 'site_resources/stylesheets/clectureprint.css" title="C tutoring Print stylesheet">');
}

var notesShowing = false;
var notesOnTip = "Turn off extra notes";
var notesOffTip = "Turn on extra notes";
...
function toggleNotes(){
	...
	parent.contents.showNotes(notesShowing);
// innerHTML set makes button gets weirdly small in Mozilla
	var notesButton = document.getElementById("hiddenNotes");
	notesButton.childNodes[0].nodeValue = (notesShowing ? "hide" : "show");
	notesButton.title = (notesShowing ? notesOnTip : notesOffTip);
	return true;
}
/**************************************************************************
...
//	printW = window.open(getToWebWriter()+"printForm.htm", "printForm","");
//	printW.focus();
}


/*function printFormLoaded(){
	parent.contents.printFormLoaded(printW);
}*/
...
function showShadeButton(show){
	var shadeButton = document.getElementById("shade");
	shadeButton.style.display = (show ? "inline" : "none");
	isShade = false;
	shadeButton.title = shadeOffTip;
}
...
function toggleShade(){
	...
	document.getElementById("shade").title = (isShade ? shadeOnTip : shadeOffTip);
	return true;
}

var lectureMode = startInLectureMode;
var lectureTip = "Lecture mode is designed for projection";
var studyTip = "Study mode is for individual viewing";
...
//alert("here");
	...
	if (lectureMode) {
		parent.contents.setStyleSheet( "lecture" );
	// innerHTML set makes button gets weirdly small in Mozilla
		modeButton.childNodes[0].nodeValue = "notes";
		modeButton.title = studyTip;
		if (notesShowing) toggleNotes();
		showShadeButton(true);
	}
	else {
		parent.contents.setStyleSheet( "notes" );
	// innerHTML set makes button gets weirdly small in Mozilla
		modeButton.childNodes[0].nodeValue = "lecture";
		modeButton.title = lectureTip;
		if (!notesShowing) toggleNotes();
		showShadeButton(false);
		parent.sliderFrame.activate(false);
	}
