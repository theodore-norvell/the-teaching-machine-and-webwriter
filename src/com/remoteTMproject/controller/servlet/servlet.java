	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	    ...
	    if(path.equals("/createRemoteTM")){
	       ...
	       // public static final int READY = 4 ;
			...
			Assert.check(rtm.evaluator.getStatusCode() == TMStatusCode.COMPILED );
			rtm.evaluator.initialize();
			Assert.check( rtm.evaluator.getStatusCode() == TMStatusCode.READY );
			if(rtm.evaluator.getStatusCode() == TMStatusCode.READY){
		...
		if(path.equals("/state")){
			...
			if(  map.getMap(1).evaluator.getStatusCode() == TMStatusCode.READY ) {
				map.getMap(1).evaluator.goForward();
		...
		if(path.equals("/expression")){
			...
			String exp = map.getMap(1).evaluator.getExpression() ;
			out.println(exp);
		}
